...
const Feedback = () => {
    useGSAP(() => {
        ...
        gsap.set(".feedback", { autoAlpha: 1, visibility: 'visible', xPercent: -90 });

        const initialAnimation = gsap.to(".feedback", {
            xPercent: 0,
            ease: "sine.out",
            scrollTrigger: {
                trigger: '.feedback',
    ...
    return (
        <div>
            <span className='bg-customBlue rounded-full px-8 py-2 hover:cursor-pointer font-bold'>
                TESTIMONIALS
            </span>
            <div className='flex sm:flex-row flex-col flex-1 gap-4 mt-12 feedback invisible'>
                {feedbacks.map((feedback, index) => (
                    <div key={index} className="sm:w-1/3 w-full overflow-hidden text-center bg-white rounded  text-slate-500 border-slate-100  border-2">
                        <div className="relative sm:p-6 p-1">
                            <figure className="relative z-10">
                                <blockquote className="p-6 text-lg leading-relaxed">
                                    <p>{feedback.text}</p>
                                </blockquote>
                                <figcaption className="flex flex-col items-center gap-2 p-6 pt-0 text-sm text-customBlue">
